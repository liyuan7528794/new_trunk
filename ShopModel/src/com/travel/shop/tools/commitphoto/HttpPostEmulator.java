...
public class HttpPostEmulator {
	...
	public String sendHttpPostRequest(String serverUrl, String path, ArrayList<FormFieldKeyValuePair> generalFormFields,
		...
		connection.setRequestProperty("Cookie", UserSharedPreference.getCookie());
		...
		for (FormFieldKeyValuePair ffkvp : generalFormFields) {
			contentBody.append("\r\n").append("Content-Disposition: form-data; name=\"").append(ffkvp.getKey() + "\"")
		...
		for (UploadFileItem ufi : filesToBeUploaded) {
			...
			// 开始真正向服务器写文件
			File file = new File(path + ufi.getFileName());
