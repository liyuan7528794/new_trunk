...
public class VideoVoteDao {
    private VideoVoteSqlHelper myDBHelper;
    public String tag = "PersonDao.class";
    private String tabeName;

    //在new出来的时候就实现myDBHelper初始化
    public VideoVoteDao(Context context, String id) {
        tabeName = "vote_" + id;
        myDBHelper = new VideoVoteSqlHelper(context, tabeName);
    }
...
//            database.execSQL("select * from " + tabeName +"='"+videoId+"'");
    ...
    public void deletePerson(String phone) {
        SQLiteDatabase database = myDBHelper.getWritableDatabase();
        if (database.isOpen()) {
            database.execSQL("delete from " + tabeName +" where phone=?", new Object[]{phone});
        }
        database.close();
    }
